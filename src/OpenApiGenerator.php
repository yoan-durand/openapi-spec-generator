...
class OpenApiGenerator
{
    ...
    public function generate($serverKey): string
    {
        ...
        foreach ($jsonapiServer->schemas()->types() as $schemaName) {
            ...
            foreach ($methods as $method) {
                ...
                if ($method !== 'GET' && ! empty($schema->fields())) {
                    ...
                    $allRequests[$schemaName."_".strtolower($method)] = new RequestBody([
        ...
        $openapi->components->__set('schemas',
          array_merge($this->getDefaultSchema(), $allSchemas));
        $openapi->components->__set('requestBodies', $allRequests);
        $openapi->components->__set('parameters',
          array_merge($openapi->components->parameters, $allParameters));
    ...
    private function getOpenApiSchema(
      ...
      $forIncludes = false
        ...
        if (isset($allSchemas[$schemaNamePlural])) {
            return $allSchemas[$schemaNamePlural];
        }
        $fieldSchemas = [];
        $relationSchemas = [];
